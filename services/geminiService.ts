...
import { AssessmentResult, LessonContext, CEFRLevel, TranslationResult, StoryScenario } from '../types';
...
const MOCK_ASSESSMENT_RESULT: AssessmentResult = {
  ...
  naturalnessScore: 7,
  complexityScore: 8,
  feedback: "[DEMO] This is a simulated assessment because the API quota is exhausted. Your sentence structure is solid but could be more idiomatic.",
  correction: null,
...
const MOCK_STORY_SCENARIO: StoryScenario = {
    id: 'mock_story_1',
    topic: 'Travel',
    situation: '[DEMO MODE] You are checking into a boutique hotel in Paris. The receptionist cannot find your reservation.',
    agentName: 'Sophie',
    openingLine: "Bonjour! Welcome to Le Grand Hotel. I am looking for your booking, but I don't see it in the system...",
    openingLineVietnamese: "Xin chào! Chào mừng đến với Le Grand Hotel. Tôi đang tìm đơn đặt phòng của bạn, nhưng không thấy trên hệ thống...",
    difficulty: 'B1-B2',
    hints: {
...
// --- RETRY LOGIC HELPER ---
const retryOperation = async <T>(operation: () => Promise<T>, maxRetries = 1, initialDelay = 1000): Promise<T> => {
    let lastError: any;
    ...
    for (let i = 0; i < maxRetries; i++) {
        try {
            ...
            const isRateLimit = msg.includes('429') || msg.includes('RESOURCE_EXHAUSTED') || msg.includes('quota') || error?.status === 429;
            
            if (isRateLimit) {
                 console.warn(`[Gemini Service] Rate limit hit. Retrying attempt ${i+1}/${maxRetries} in ${delay}ms`);
...
const assessmentSchema: Schema = {
  type: Type.OBJECT,
  properties: {
    score: { type: Type.NUMBER, description: "Overall score on a scale of 1-10 based on CEFR mapping." },
    accuracyScore: { type: Type.NUMBER, description: "Score for grammatical accuracy (1-10)." },
    naturalnessScore: { type: Type.NUMBER, description: "Score for pragmatic naturalness (1-10)." },
    complexityScore: { type: Type.NUMBER, description: "Score for vocabulary complexity (1-10)." },
    feedback: { type: Type.STRING, description: "General constructive feedback." },
    grammarAnalysis: { type: Type.STRING, description: "Specific analysis of grammatical errors. Start with 'Grammar Point:'." },
    vocabularyAnalysis: { type: Type.STRING, description: "Specific analysis of vocabulary choice/appropriateness. Start with 'Vocabulary Point:'." },
    ...
    alternativeTones: {
      type: Type.OBJECT,
      properties: {
        formal: { type: Type.STRING, description: "A formal/business appropriate version." },
        friendly: { type: Type.STRING, description: "A warm, friendly version." },
        informal: { type: Type.STRING, description: "A very casual or slang version." },
        conversational: { type: Type.STRING, description: "A standard, neutral conversational version." }
      },
...
const lessonSchema: Schema = {
  ...
  properties: {
    ...
    englishPhrase: { type: Type.STRING, description: "The English translation of the Vietnamese phrase, matching the tone." },
  ...
  required: ['id', 'title', 'situation', 'vietnamesePhrase', 'englishPhrase', 'difficulty', 'hints']
};
...
const storySchema: Schema = {
    ...
    properties: {
        ...
        situation: { type: Type.STRING, description: "Detailed background of the conversation scene." },
        agentName: { type: Type.STRING, description: "Name of the character the AI is playing." },
        openingLine: { type: Type.STRING, description: "The first sentence spoken by the Agent (in English)." },
        openingLineVietnamese: { type: Type.STRING, description: "Vietnamese translation of the opening line." },
        difficulty: { type: Type.STRING, enum: ['A1-A2', 'B1-B2', 'C1-C2'] },
        hints: {
            type: Type.OBJECT,
            properties: {
                level1: { type: Type.STRING, description: "Hint about the meaning/intent of how to reply." },
                level2: { type: Type.STRING, description: "Hint about the sentence structure." },
                level3: { type: Type.STRING, description: "Hint about key vocabulary to use." },
            },
            required: ['level1', 'level2', 'level3']
        }
    },
    required: ['id', 'topic', 'situation', 'agentName', 'openingLine', 'openingLineVietnamese', 'difficulty', 'hints']
};
...
const translationSchema: Schema = {
  ...
  properties: {
    ...
    tones: {
      type: Type.OBJECT,
      properties: {
        formal: { type: Type.STRING, description: "Formal, business-appropriate version." },
        friendly: { type: Type.STRING, description: "Warm, friendly version." },
        informal: { type: Type.STRING, description: "Casual, slang, or close-friend version." },
        conversational: { type: Type.STRING, description: "Neutral, everyday conversational version." }
      },
      required: ["formal", "friendly", "informal", "conversational"]
    }
  },
...
export const generateNextLesson = async (
    ...
    const levelSpecs = {
        ...
        'C1-C2': 'Sentence Length: 15-22+ words. Vocabulary: Nuanced, abstract. Focus: Persuasion, hypothesis, subtle humor.'
    };
...
export const generateScenarioVideo = async (situation: string, phrase: string): Promise<string> => {
    ...
    try {
        let operation = await retryOperation<any>(() => veoAi.models.generateVideos({
        ...
        while (!operation.done) {
            ...
            operation = await retryOperation<any>(() => veoAi.operations.getVideosOperation({operation: operation}));
