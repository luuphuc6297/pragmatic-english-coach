...
const SavedItemsModal: React.FC<SavedItemsModalProps> = ({ items, onClose, onDelete, onPractice, onSync }) => {
  ...
  return (
    <div className={styles.modal.overlay}>
      <div className={`${styles.modal.container} max-w-2xl max-h-[80vh] animate-in zoom-in-95 duration-200`}>

        {/* Header */}
        <div className={`${styles.modal.header} bg-slate-50/50`}>
          <div>
          ...
          <div className="flex items-center gap-2">
            {onSync && (
              <button
                ...
                className={`${styles.button.ghost} hover:text-slate-600 disabled:opacity-50`}
              >
                {syncSuccess ? (
            ...
            <button
              onClick={onClose}
              className={`${styles.button.ghost} hover:text-slate-600`}
            >
              ✕
            </button>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-slate-50">
          ...
          {items.length > 0 && (
            <div className="flex flex-col gap-4 mb-6">
              <div>
                <h4 className={`${styles.text.label} mb-2`}>Categories</h4>
                <div className="flex flex-wrap gap-2">
                  {categories.map((cat) => (
                    <button
                      key={cat}
                      onClick={() => setSelectedCategory(cat)}
                      className={`${styles.badge.base} transition-colors ${
              ...
              <div>
                <h4 className={`${styles.text.label} mb-2`}>Part of Speech</h4>
                <div className="flex flex-wrap gap-2">
