...
const Onboarding: React.FC<OnboardingProps> = ({ onComplete }) => {
  ...
  const handleLevelSelect = (l: CEFRLevel) => {
    setLevel(l);
  };
    ...
    <div className="flex flex-col items-center justify-center min-h-screen bg-slate-900 p-6 text-white relative overflow-hidden">
      ...
      <div className="max-w-3xl w-full z-10">
        ...
        {step === 1 && (
            <div className="animate-in slide-in-from-right fade-in duration-500">
                ...
                <div className="mb-8 max-w-md mx-auto">
                    <label className="block text-sm font-bold text-slate-400 uppercase tracking-wider mb-2 text-center">What should we call you?</label>
                    <input 
                        type="text" 
                        value={name}
                    ...
                    {ONBOARDING_LEVELS.map((lvl) => (
                        ...
                        >
                            <div className="flex justify-between items-start mb-2">
                                <span className={`text-2xl font-black ${level === lvl.id ? 'text-brand-400' : 'text-slate-500'}`}>
                                    {lvl.id}
                                </span>
        ...
        <div className="mt-12 flex justify-center">
            <button
                onClick={handleNext}
                disabled={(step === 1 && (!level || !name.trim())) || (step === 2 && topics.length === 0)}
                className={`
        ...
        <div className="flex justify-center gap-2 mt-8">
            <div className={`w-2 h-2 rounded-full transition-colors ${step >= 1 ? 'bg-brand-500' : 'bg-slate-700'}`} />
            <div className={`w-2 h-2 rounded-full transition-colors ${step >= 2 ? 'bg-brand-500' : 'bg-slate-700'}`} />
        </div>
      </div>
    </div>
  );
