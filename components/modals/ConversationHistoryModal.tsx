          ...
          {filteredHistory.length === 0 ? (
            ...
            <div className="space-y-4">
              {filteredHistory.map((item) => (
                <div
                  key={item.id}
                  className={`${styles.card.interactive} flex items-start justify-between`}
                  onClick={() => onSelect(item)}
                >
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      <span className={`${styles.badge.status} ${item.mode === 'roleplay' ? 'bg-emerald-100 text-emerald-700' :
